...
/// <summary>
/// Absztrakt ős olyan adapter osztályok számára,
/// melyek valamilyen típusú felületelemet kötnek össze CommandBinding objektumokkal.
/// </summary>
public abstract class CommandBindingConnectorBase
{
    protected CommandBinding CommandBinding { get; }
    ...
    protected CommandBindingConnectorBase(CommandBinding commandBinding)
    {
        ...
        CommandBinding.EnableChanged += HandleEnableChangedEvent;
        CommandBinding.VisibleChanged += HandleVisibleChangedEvent;
        CommandBinding.SelectedChanged += HandleSelectedChangedEvent;
    }
}

/// <summary>
/// Adapter osztály CommandBinding objektumok ToolStripMenuItem (vagyis menüelem) objektumokkal való összekötésére.
/// </summary>
public class ToolStripMenuItem_CommandBindingConnector : CommandBindingConnectorBase
{
    private ToolStripMenuItem m_item;

    protected ToolStripMenuItem_CommandBindingConnector(ToolStripMenuItem item, CommandBinding commandBinding)
        : base(commandBinding)
    {
        m_item = item;
        m_item.Click += HandleUIEvent;
    }
    ...
    {
        var _ = new ToolStripMenuItem_CommandBindingConnector(item, commandBinding);
    }
}
